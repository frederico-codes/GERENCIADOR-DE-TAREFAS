...
export class TasksController {
  async createTask(req: Request, res: Response): Promise<void> {
    ...
    if (!teamMember) {
      throw new AppError("O usuário atribuído não pertence a essa equipe.", 400)
    }
    ...
    try {
      const newTask = await prisma.task.create({
        ...
        include: {
          assignee: {
            //assignee campo obrigatório no insomnia.objeto Usuário completo
            select: {
              name: true,
            },
          },
  ...
  async updateTask(req: Request, res: Response): Promise<void> {
    ...
    const bodySchema = z.object({
      title: z
        .string()
        .trim()
        .min(1, "O titulo da tarefa e obrigatorio.")
        .optional(),
      description: z
        .string()
        .trim()
